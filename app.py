...
with tab_info:
    ...
    2026年2月10日、愛波恒平が New York にて旅立ちました。
    ...
    **愛波恒平の人生を  みんなでお祝いする会**
    ...
    st.info("""
    **📍 日時:** 2026年3月21日（土）16:30 - 20:00（出入り自由）  
    ...
    **📍 会費:** 6,000円（お子さんは無料）  
    ほんの少しだけ顔を出す、という方は当日ご相談ください。
    """)
    
    st.markdown("""
    #### 💰 お支払いについて
    
    事前に **PayPay** でのお支払いをお願いしています。  
    難しい方は **銀行振込**、当日 **PayPay** または **現金** でも大丈夫です。
    
    - **PayPay:** [PayPayで支払う](https://qr.paypay.ne.jp/p2p01_bMDJSvSDraHqGVhq)
    - **振込先:** 三菱UFJ銀行（府中支店）／口座番号：1749387／名義：愛波理恵子
    
    ※PayPayも振込も、送金の際には **フルネーム** をご記入ください（PayPayのニックネームだけだと分からないことがあるため）
    """)
    ...
    恒平の友人・仲間のみなさんのつながりの広さゆえ、参加人数が想定以上に増えてきました。本当にありがとうございます。
    
    会場のキャパシティを超えてしまう可能性があるため、2点ご協力をお願いします。
    
    1. **来場時間の分散にご協力ください**  
    16:30開始の時間帯にご来場が集中しています。会は20:00まで行っていますので、可能な方は後半のお時間に分散してお越しいただけると助かります。
    
    2. **向かいにもお席を用意しました**  
    会場が混み合った場合に備えて、向かいの「焼き鳥 ひろや」にもお席を用意しました。状況によっては、向かい側に移動していただき、そのままお祝いを続けていただく可能性があります。  
    📍 [焼き鳥 ひろや](https://maps.app.goo.gl/DePzsH53WXFkaz8n9)
    """)
    ...
    st.link_button("📝 参加フォームに回答する", "https://forms.gle/UxH9bMdW55xeA7kM7", use_container_width=True)
    ...
    **🔗 ドネーションは、Fundタブからお願いいたします。**
    ...
    当日現地に来られない方も、**Live Stream（Zoom）** でオンライン参加が可能です。  
    Zoomリンクは、参加登録フォームで「Live Stream（オンライン）で参加」を選択された方にメールでお送りします。
    
    👉 **オンライン参加をご希望の方は、事前に参加登録をお願いします。**
    """)
    
    st.link_button("📝 参加フォームに回答する", "https://forms.gle/UxH9bMdW55xeA7kM7", use_container_width=True)
    
    st.markdown("""
    見るだけの参加も大丈夫です。タイミングが合えば、画面越しに会場へメッセージをお話しいただくこともできます。  
        ...
        for idx, song_row in unique_songs.iloc[::-1].iterrows():
            ...
            with st.container():
                ...
                with col_left:
                    ...
                    st.link_button("▶️ YouTubeで聴く", yt_url)
...
with tab_live:
    ...
    st.info("恒平の長男Kenzoくんが葬儀のために作成した映像です。")
    ...
    st.markdown("### 📺 Live Stream（オンライン参加）")
    st.info("""
    当日は **Zoom** によるライブ配信を予定しています。  
    会場の様子や、仲間からの思い出シェアをオンラインでお楽しみいただけます。
